...
compileResults <- function(object, padj=0.05, zScore=3, 
    ...
    if(!all(features %in% assayNames(object))){
        stop(paste("Some of the assays are missing. Please run the full", 
    ...
    if(all==FALSE){
        ...
        if(dim(object)[1]==0){
            warning('No significant count use all=TRUE to print all counts.')
        }
    }
    ...
    featureDTs <- append(featureDTs, 
                         list(rawcounts=countdataDF, normcounts=countNormDF))
    ...
    tidyresults <- merge(tidyresults, 
                         data.table(sampleID = colnames(object), 
                                    AberrantBySample = aberrant(object, by='sample')), 
                         by = c('sampleID'))
    
    tidyresults <- merge(tidyresults, 
                         data.table(geneID = rownames(object), 
