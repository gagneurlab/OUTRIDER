...
\Rfunction{gridSearchEncDim} function. 
...
(Benjamini and Yekutieli, 2001). Alternatively, all other by 
\Rfunction{p.adjust} supported methods can be used via the \Robject{method} 
parameter.
