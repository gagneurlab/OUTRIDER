...
It does so by fitting a negative binomial model on RNA-seq read counts, correcting for variations in
sequencing depth and apparent co-variations across samples. Read counts significantly deviating 
...
significant differences between groups of samples. In contrast, \outrider{} aims at detecting
...
the python environment was installed into \Rfunction{~/python-env-OUTRIDER}.
...
ods <- makeExampleOutriderDataSet(50, 70)
...
# Loading
ods <- makeExampleOutriderDataSet(100, 100)
ods <- estimateSizeFactors(ods)
