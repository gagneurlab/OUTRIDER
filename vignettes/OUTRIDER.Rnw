...
now (June 2018), we recommend the STAR aligner. After obtaining the aligned BAM
...
pipeline using the command \Rfunction{OUTRIDER}. In the third and last step the result
...
\italic{et al}. For testing, this package contains also a subset of it.
...
that we observer at least 1 read every 100 samples. Therefore,
...
low FPKM (Fragments Per Kilo base per Million reads) expression values. The needed annotation to estimate FPKM values 
...
# calculate FPKM values and only label not expressed genes
...
Therefore an encoding dimension q needs to be set or the default value 20 is used.
The optimal value of q can be determinde using the \Rfunction{gridSearchEncDim} function. 
After controlling for co-founders, the heatmap should be plotted again.
