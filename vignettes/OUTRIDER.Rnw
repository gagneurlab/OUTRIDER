...
Since this function runs a full OUTRIDER fit for a variety of encoding dimensions, 
it is quite CPU-intensive. Thus, it is recommended to use the faster function \Rfunction{estimateBestQ} which relies on Optimal Hard Thresholding\cite{Gavish2013}.
It is the default method to find the optimal encoding dimension if \textit{q} is 
