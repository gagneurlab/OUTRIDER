...
introductions of \deseq{}\cite{Love2014}, \Biocpkg{edgeR}\cite{Robinson2010} or the RNA-seq workflow from
Bioconductor: \Biocpkg{rnaseqGene}. In brief, one usually starts with the raw FASTQ files from
the RNA sequencing run. Those are then aligned to a given reference genome. At the time of writing
...
Apart from running the full pipeline using the single wrapper function \Rfunction{OUTRIDER}, 
...
\textit{et al}.\cite{Kremer2017}. For testing purposes, this package contains a small
subset of it.

\subsection{OutriderDataSet}
To use \outrider{} create an \ods, which derives from a RangedSummarizedExperiment
...
that we observe at least one read for every 100 samples. Therefore, all genes that are not 
...
However, the \Robject{TxDb.Hsapiens.UCSC.hg19.knownGene} contains only well annotated 
...
can be observed. These are often due to technical confounders (e.g. sequencing batch) 
or biological confounders (e.g. sex, age). These confounders can adversely affect the 
...
# we use only 5 iterations to make the vignette faster. The default is 15. 
ods <- estimateSizeFactors(ods)
ods <- controlForConfounders(ods, q=21, iterations=5)
...
\subsubsection{Using PEER to control for confounders}
...
of each other, replicates could mask effects specific to this sample. This is 
also true if trios are present in the data, where the parents can be seen as
biological replicates. Here, we recomment to mask the sample of interest or 
the replicats during the autoencoder fit. Later on, for all samples P-values 
are calculated. 

In this rare disease data set we know that two samples (MUC1344 and MUC1365)
have the same defect. To exclude one or both of them we could use
