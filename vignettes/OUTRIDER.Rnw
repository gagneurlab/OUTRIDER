...
to RNA-seq read counts while correcting for variations in sequencing depth and 
...
the RNA sequencing run. Those are then aligned to a given reference genome. Currently
(June 2018), we recommend the STAR aligner\cite{Dobin2013}. After 
...
preprocessing needs to be done. We can start and obtain results using 
3 commands. First, create an \ods{} from a count table. Second, run the full 
pipeline with the command \Rfunction{OUTRIDER}. In the third and last step the 
...
samples that fail quality measurements should be done manually before running the 
...
\textit{et al}.\cite{Kremer2017} A small subset is contained in this package 
for testing.
...
or biological confounders (e.g. sex, age). These confounders can adversly affect the 
