...
As of now (May 2018), we recommend the STAR aligner. After obtaining the aligned
BAM files, one can map the reads to exons or genes of a GTF annotation file 
using HT-seq. The resulting count table can then be loaded into the \outrider{} 
package as we will describe below.
