...
\newcommand{\outds2}{\Rclass{Outrider2DataSet}}
...
The \outds2{} is a generalization of the \ods{} class. Input values no longer have 
to be counts/integers, but can be continuous values and might also contain some 
missing values (NAs). When creating the \outds2{}, the correct profile should be 
...
intensities e.g. from proteomics measurements (see TODO for more details), 
...
As \ods{} is now a subclass of \outds2{}, the profile is automatically set to 
...
'py\_outrider' package which can be installed from pip by running ... (TODO). 
...
`prepro\_opt[["data\_trans"]] <- "log"`. Here, however, currently only 
'log', 'log1p' or NULL (no transformation) are supported. 
Also, sizefactor normalization can be turned on or off by setting 
`prepro\_opt[["sf\_norm"]] <- TRUE/FALSE`.
By default, the distribution specified in `prepro\_opt[["distribution"]]` 
...
in this case, the argument `loss\_distribution` should be supplied to the 
`OUTRIDER()` function to specify which distribution to use for the model 
loss.

\subsection{Including known confounders in th model fitting}
To explicitly consider confouders known beforehand, the user only has to 
specify the respective columns in the `colData` of the \outds2{}. The column 
...
ods <- OUTRIDER(ods, covariates = c("batch"))
@
...
Furthermore, the default cutoffs on adjusted p values and effects (fold change, 
zscores, or delta values, if appropriate) might be adjusted according to the 
user's preferences.
...
used to visualize \outds2{} results. The name of some feature-level functions 
has been adapted to support the more general \outds2{} objects:  
`plotExpressionRank` -> `plotFeatureRank`, 
