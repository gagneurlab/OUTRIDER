...
\Rcode{prepro_opt[["data_trans"]] <- "log"}. Here, however, currently only 
'log', 'log1p' or \Rcode{NULL} (no transformation) are supported. 
Also, sizefactor normalization can be turned on or off by setting 
\Rcode{prepro_opt[["sf_norm"]] <- TRUE/FALSE}.
By default, the distribution specified in \Rcode{prepro_opt[["distribution"]]} 
...
in this case, the argument \Rcode{loss_distribution} should be supplied to the 
